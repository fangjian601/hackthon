...
public class MovingWindowExample {
	...
	private static Logger log = LoggerFactory
			.getLogger(MovingWindowExample.class);

	public static void main(String[] args) throws Exception {

		InputStream is = new ClassPathResource("res/sentiment-tweets.small.csv")
				.getInputStream();
		LabelAwareSentenceIterator iterator = new LabelAwareListSentenceIterator(
				is, ",", 1, 3);
		// get rid of @mentions
		...
		Word2Vec vec = vecModel.exists() ? (Word2Vec) SerializationUtils
				.readObject(vecModel) : new Word2Vec(tokenizerFactory,
				iterator, 5);
		if (!vecModel.exists()) {
			vec.train();
			...
			SerializationUtils.saveObject(vec,
					new File("tweet-wordvectors.ser"));
		}
		...
		DataSetIterator iter = new Word2VecDataSetIterator(vec, iterator,
				Arrays.asList("0", "1", "2"));
		if (!iter.hasNext())
			throw new IllegalStateException("No data found");

		/*
		 * Note that this is an example of how to train. The parameters are not
		 * optimally tuned here, but serve to demonstrate how to use bag of
		...
		c.setLayerSizes(new int[] { iter.inputColumns() / 4,
				iter.inputColumns() / 4, iter.inputColumns() / 3 });
		...
		runner.setModelSaver(new DefaultModelSaver(new File(
				"word2vec-modelsaver.ser")));
		...
		while (iter.hasNext()) {
			...
			Pair<Double, DoubleMatrix> decoded = viterbi.decode(next
					.getSecond());
